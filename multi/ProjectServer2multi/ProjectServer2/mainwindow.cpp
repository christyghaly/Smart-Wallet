...
void MainWindow::ThreadCaller(int x,int socketfd,QMutex* mut)
{
               ...
               while (noThread<3)
               {
                 ...
                 socklen_t  len = sizeof(clntAdd);
                   ...
                   //this is where client connects. svr will hang in this mode until client conn
                       ...
                       ThreadCaller(noThread,connFd1,mut);
    ...
    while(!loop)
    {
        ...
        if(l.contains(",")){
            ...
            if(list1.contains("deposit")){
               ...
               bool locked= mutptr->try_lock(); // try to lock on the ui update to prevent data racing on the status bar 
                char*out;
                char*resptr;
                if(locked){
                 emit update(l,threadNo);
...
char* deposit_withdraw(char*resptr,QStringList L, int type)  /// this function allow deposition / withdrawing / donation and pay telephone bill
{
    ...
    while(!File.eof() && found==false)
    {
        ...
        if(ac.retacno()==n)
        {
            ...
            newstring.append("$");
            get_pos("allaccount.txt",tranFile,newstring);
